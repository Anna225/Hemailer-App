...
class ChatHistoryScreen extends StatefulWidget {
  ...
  ChatHistoryScreen({Key key, @required this.userInfo, this.clientInfo, this.chatSocket}) : super(key: key);
  @override
...
class _ChatHistoryScreenState extends State<ChatHistoryScreen>{
  ...
  void initState(){
    ...
    ApiService.getChatHistory(body).then((response) {
      print(response["hist_data"]);
      setState(() {
  ...
  void receiveMSG(){
    widget.chatSocket.addListener((){
      dynamic newMsg = widget.chatSocket.recentMsg;
  ...
  void sendMSG(){
    var messageJSON = {
      ...
      "parentID": widget.userInfo.parentID,
      "msg_owner": "agent",
      "attach_url": "",
      "attach_real_name": "",
    };
    print(messageJSON);
    if(_txtMsg.text != ""){
      widget.chatSocket.sendMsg(messageJSON);
    }
  }
  void readMsg(){
    print("readMsg");
    ...
    print(body);
    ApiService.readChatMsg(body).then((response) {
  ...
  Future<void> deleteClientHistory() async {
    ...
    if(action == ConfirmAction.YES){
      ...
      ApiService.deleteChatHistory(body).then((response) {
        if (response != null && response["status"]) {
          setState(() {
            allChatMsg.clear();
            msgCount = 0;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text("Chat with " + widget.clientInfo["username"]),),
        ...
        child:Container(
          ...
          child: Column(
            children: <Widget>[
              Card( //                           <-- Card widget
                child: ListTile(
                  title: Text(widget.clientInfo["email"]),
                  subtitle: Padding(
                    ...
                    child: Container(
                      ...
                      decoration: new BoxDecoration(
                        ...
                        image: new DecorationImage(
                          image: NetworkImage(widget.clientInfo["photo_url"] == "" ? baseURL + 'uploads/avatar/profile.jpg' : baseURL + widget.clientInfo["photo_url"]),
                        ),
